 ...
 * @version     V1.0
 * @date        2026-02-04
 * @brief       LCD 驱动与基础绘图接口（FSMC 8080 并口）
 * @details
 *  本文件提供 LCD 的底层访问与基础绘图能力，主要包括：
 *   - LCD 寄存器/GRAM 读写：lcd_wr_regno() / lcd_wr_data() / lcd_rd_data()
 *   - 基本控制：lcd_init() / lcd_display_on() / lcd_display_off()
 *   - 坐标与窗口：lcd_set_cursor() / lcd_set_window() / lcd_scan_dir() / lcd_display_dir()
 *   - 绘图与填充：lcd_clear() / lcd_fill() / lcd_color_fill()
 *   - 图元：lcd_draw_point() / lcd_draw_line() / lcd_draw_rectangle() / lcd_draw_circle() 等
 *   - 字符显示：lcd_show_char() / lcd_show_num() / lcd_show_string()
 *
 *  驱动兼容说明（通过读 ID 自动识别并初始化）：
 *   - ILI9341(0x9341), ST7789(0x7789), NT35310(0x5310), ST7796(0x7796)
 *   - NT35510(0x5510), ILI9806(0x9806), SSD1963(0x1963)
 *
 *  注意事项：
 *   1) 本驱动通过 FSMC 访问 LCD（8080 时序），依赖 CubeMX/HAL 的 FSMC 配置。
 *   2) lcd_init() 内部曾包含 printf 打印 ID；若 USART1 未初始化可能导致阻塞，请谨慎开启。
